...
const apiURL = "/api/v1/san-pham-chi-tiet/thuong-hieu";
let _existingNames;
$(document).attr("title", "Quản lý thương hiệu")
        ...
        "columns": [
            ...
            {"data": "ten"},
            {
                ...
                "render": function (data, type, row) {
                    return `
                        <img style="width: 100px;" src="${data}"/>
                    `
                }
            },
            {
...
$(document).ready(()=>{
    $('input[type="color"]').on('change, input',function (){
        $(this).closest('div').find('label').html(('thương hiệu: '+$(this).val()).toUpperCase())
    })
})
...
$(document).ready(() => {
    ...
    $('#dataTable tbody').on('click', '.btn-edit', function () {
        ...
        if (rowData) {
            ...
            let id = rowData.id;
            let ten = rowData.ten;
            let thumbnail = rowData.thumbnail;
            let isDeleted = rowData.isDeleted;

            // Binding dữ liệu vào modal
            $('#edit-id').val(id);
            $('#edit-ten').val(ten);
            $('#image-edit-preview').attr('src',thumbnail);
            $('#edit-deleted').html(`
        ...
        return new Promise((resolve, reject) => {
            for (let obj of jsonData) {
                ...
                if (!obj.hasOwnProperty('ten')) {
                    reject("Lỗi định dạng: Thiếu tên thương hiệu")
                }
            }
            resolve(jsonData)
        })
